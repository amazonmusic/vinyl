    ...
    implements AdController
{
    ...
    setAdBreaks(adBreaks: readonly AdBreakInfo[]): void {
        ...
        // If the previously active break is gone, treat it as an exit.
        if (this._active && !sorted.some((b) => b.id === this._active!.id)) {
            const exited = this._active
            this._active = null
            this.dispatch('adBreakExit', exited)
        }
    }
    ...
    updateTime(currentTime: number): void {
        ...
        this._active = next
        if (previous) this.dispatch('adBreakExit', previous)
        if (next) this.dispatch('adBreakEnter', next)
    }

    /**
     * Clears all state. Emits an `adBreakExit` if a break was active so
     * listeners observe a clean exit when the media is unloaded.
     */
    dispose(): void {
        const active = this._active
        this._active = null
        this._adBreaks = []
        if (active) this.dispatch('adBreakExit', active)
        super.dispose()
    }
