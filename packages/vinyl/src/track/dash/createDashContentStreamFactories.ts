...
/**
 * Dependencies needed for a new content stream provided by the dash track.
 */
export interface DashContentStreamTrackDeps {
 ...
 */
export function createDashContentStreamFactories(
    options: Maybe<ContentStreamingOptions>
) {
    return (deps: DashContentStreamTrackDeps) =>
        memoize(
