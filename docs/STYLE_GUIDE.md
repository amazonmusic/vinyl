...
Do not discard a promise with `void`. A floating promise swallows rejections, so
a failure surfaces (if at all) only as an unhandled rejection with no context.
Attach a `.catch` handler instead so the error is routed somewhere meaningful.
Prefer the module's/application's existing error handler over a no-op catch.
...
```typescript
void resource.load().then((value) => use(value))
```
